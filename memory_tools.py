...
- {"tool": "update_tasks", "content": "全文"}
  update_tasks: ユーザーのタスクリスト（tasks.md）を全文で書き直す。新しいタスクを
  聞いたとき・状況が変わったときに整理して更新する。書式: 「## いまやる」と
  「## これから」の2見出し、1行1タスク「- 内容 ｜YYYY-MM-DD ｜カテゴリ」
  （期限・カテゴリは任意）。完了したタスクは行ごと消してよい——完了の記録は
  アプリがtasks_done.mdに残す。期限が決まっているものはset_reminderも併用する
...
_TOOL_ONELINERS = {
    ...
    "update_tasks": '- update_tasks: タスク一覧を全文書き直す（## いまやる/## これから、'
                     '1行「- 内容 ｜YYYY-MM-DD ｜カテゴリ」・完了は行ごと消す） '
                     '{"tool":"update_tasks","content":"全文"}',
...
_TOOL_DETAILS = {
    ...
    "update_tasks": ('{"tool": "update_tasks", "content": "全文"}\n'
                      'ユーザーのタスクリスト（tasks.md）を全文で書き直す。書式: '
                      '「## いまやる」と「## これから」の2見出し、1行1タスク'
                      '「- 内容 ｜YYYY-MM-DD ｜カテゴリ」（期限・カテゴリは任意・順不同）。'
                      '完了したタスクは行ごと消してよい——完了の記録はアプリが'
                      'tasks_done.mdに残すので、消しても歴史は失われない。'
