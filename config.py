...
DEFAULT_CONFIG = {
    ...
    # 定期処理の実行時刻（0-23時）の上書き。既定はscheduler.JOBSの"hour"
    # （日次0時・索引3時・週次4時・月次5時・内省6時・庭仕事7時）で、ここには
    # ユーザーが変えたジョブだけが入る。全ジョブを既定値でここに並べないのは、
    # 将来ジョブや既定時刻を変えたときに古い値が固定化されるのを避けるため。
    "scheduled_job_hours": {},
